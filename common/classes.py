...
class ContainerPaginator:
    ...
    title: str | None = attrs.field(kw_only=True, default=None)
    ...
    color_for_pages: list[ipy.Color] = attrs.field(
        repr=False, kw_only=True, factory=list
    )
    ...
    def create_components(self, disable: bool = False) -> ContainerComponent:
        ...
        output: ContainerComponent = ContainerComponent(
            accent_color=(
                self.color_for_pages[self.page_index].value
                if self.color_for_pages
                else self.bot.color.value
            ),
        )
        if self.title:
            output.append(ipy.TextDisplayComponent(f"# {self.title}"))
